...
(function () {
  ...
  const url = `https://www.roblox.com/games/${gid}`;
    let hints = ["* Quick search, skips to the end good for finding small or empty servers.","* Smart search, goes inbetween pages to find the closest playercount."];
  ...
  const fill = function(str,max,cap,fstr)
{
 ...
 const joinServer = function(placeID,UUID)
 {
     Roblox.GameLauncher.joinGameInstance(placeID,UUID);
 }
 ...
 const getRandomInt =function(min, max) {
  ...
  {
      hintText = hints[getRandomInt(0,hints.length-1)]
  }
  ...
  const updateCapacity = function()
  {
    ...
    const searchForGame2 = function (gid, min, max,search ) {
    if (!searchStatus) {
        setStatus(hintText)
        return
    }
      ...
      .then(function (data) {
        ...
        if (matchFound) {
          var server = data.Collection[matchNum];
            ...
            //searchForGame(gid, min, max,search);
            console.log(server)
          ...
          max = page;
          updateStatus(search,page,close);
          searchForGame2(gid, min, max,search);
        } else {
          min = page;
          updateStatus(search,page,close);
          searchForGame2(gid, min, max,search);
  ...
  const searchForGame = function (gid, min, max,search ) {
        if (!searchStatus) {
        setStatus(hintText)
        return
    }
  ...
  btn.onclick = function () {
    hideSearchButtons()
      resetParam()
    searchStatus = true
    searchForGame(gid, 0, searchTotal,searchCount);
  };
  ...
  btn2.id = "-meow-findServer";
  btn2.onclick = function () {
      if (searchCount == 0)
      {
          alert("Use Quick Search to easier find servers with 0 Players!");
      }
    hideSearchButtons()
      resetParam()
    searchStatus = true
    searchCountRange = [searchCount,searchCount];
    searchForGame2(gid, 0, 10000,searchCount);
  };
...
//<input type="checkbox" ng-checked="notificationBand.isEnabled" ng-disabled="isDestinationTypeOptedOut(receiverDestinationType) || !notificationBand.isOverridable" ng-click="toggleNotificationBand(notificationBand)" id="NotificationStream-checkbox-1" checked="checked">
//<li class="stack-row rbx-game-server-item"?
    //<div class="text-info rbx-game-status rbx-game-server-status">11 of 12 people max</div> ul section rbx-game-server-item-container stack-list
...
//<span class="avatar avatar-headshot-sm player-avatar">
  //<a class="avatar-card-link">
    //<img class="avatar-card-image" src="https://tr.rbxcdn.com/5721f6bf19a9fdd7860684d731c1f239/48/48/AvatarHeadshot/Png">
    //</a>
//</span>
...
//   <li class="stack-row rbx-game-server-item" data-gameid="0973ae2e-e6db-4eff-bce5-089b3580d99c">
//   <div class="section-header">
//      <div class="link-menu rbx-game-server-menu"></div>
//   </div>
//   <div class="section-left rbx-game-server-details">
//     <div class="text-info rbx-game-status rbx-game-server-status">11 of 12 people max</div>
//    <a class="btn-full-width btn-control-xs rbx-game-server-join" href="#" data-placeid="2727067538" onclick="Roblox.GameLauncher.joinGameInstance(2727067538, &quot;0973ae2e-e6db-4eff-bce5-089b3580d99c&quot;)">Join</a>
...
//</li>
    ...
    const addToServerPanels = function(server)
    {
       ...
       for (let i = 0; i < server.CurrentPlayers.length; i++) {
        ...
        // Good stuffs here! ( EVAL ) Removed !! ! !
        var button = document.getElementById(id + "joinButton");
        button.onclick = (e) => {
          window.Roblox.GameLauncher.joinGameInstance(gid,server.Guid)
        }
